...
> Reminders: **no schema change without this spec** · **migrations run as a controlled step, not app boot; backup-first** · **the event log is append-only — no destructive migration on it** · update `DATABASE.md` same-PR · **no impl before `START IMPLEMENTATION`** · **stop, don't guess** on data-loss risk. Conforms to `DATABASE.md`, `DEPLOYMENT.md`.
