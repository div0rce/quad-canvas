...
**Remaining for G2:** reconnect convergence + live end-to-end (M18–M19 — on WS reconnect re-fetch the snapshot, resubscribe, and resume deltas beyond the new watermark; verify a placement repaints a live browser). **G2 is not yet reached** — the full loop is wired; reconnect hardening + a browser e2e remain.
