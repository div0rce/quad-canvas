...
- Verified with Docker-backed integration tests: `pnpm --filter api test:integration` (21/21 — placement, the read surface, and WS subscribe/heartbeat/fan-out) and `pnpm --filter @quad/realtime test:integration` (cross-node Redis delivery); plus `@quad/realtime` + `@quad/render` unit tests.

**Remaining for G2:** wire `@quad/render` into the `apps/web` canvas (M17 — fetch snapshot, subscribe to deltas, paint), then reconnect convergence + live end-to-end (M18–M19). **G2 is not yet reached** — server live loop + client render model are done; the interactive frontend + reconnect remain.
