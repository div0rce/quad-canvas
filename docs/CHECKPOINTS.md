...
- Verified with Docker-backed integration tests: `pnpm --filter api test:integration` (21/21 — placement, the read surface, and WS subscribe/heartbeat/fan-out) and `pnpm --filter @quad/realtime test:integration` (cross-node Redis delivery); plus `@quad/realtime` unit tests (registry + bus).

**Remaining for G2:** `@quad/render` + `apps/web` canvas (M16–M17), reconnect convergence + live end-to-end (M18–M19). **G2 is not yet reached** — the server side of the live loop is done; the client render + reconnect remain.
